...
(function($){
	...
	Enlarge.prototype = {
		...
		"createHTML":function(){
			...
			//创建包括小图，给定其src，并将它添加到小div中，再给它添加行内样式
...
//			
			//创建包括大图，给定其src，并将它添加到大div中，再给它添加行内样式
...
//			
		...
		"monitor":function(){
			...
			small.on("mousemove",function(e){
				...
				$big.parent().css({"display":"inline-block"})
				.end().css({
				...
				//水平右边界判断
					...
					$track.css({"left":$track.parent().width()-$track.width()});
					$big.css({"left":$big.parent().width()-$big.width()});
				...
				//垂直下边界判断
					...
					$track.css({"top":$track.parent().height()-$track.height()});
					$big.css({"top":$big.parent().height()-$big.height()});
